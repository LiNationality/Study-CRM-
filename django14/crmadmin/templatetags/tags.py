...
def build_table_row(request,obj,admin_class):
    ...
    for index,column in enumerate(admin_class.list_display):
        ...
        # column_data=getattr(obj,column)
        if index==0:#跳转到修改页
            column_data="<a href='{request_path}{obj_id}/change/'>" \
                        "{data}</a>".format( request_path= request.path,
                                                                                             obj_id = obj.id ,
                                                                                              data =column_data
                                                                                             )
...
def render_page_ele(loop_counter,query_sets,filter_condtions):
    filters=''
    for key ,value in filter_condtions.items():
        filters += "&%s=%s" %(key, )
...
def render_filter_ele(filter_field,admin_class,filter_condtions):
    ...
    if type(field_obj).__name__=="ForeignKey":
        ...
        for choice_item in field_obj.get_choices()[1:]:
            ...
            select_ele+='''<option value='%s' %s>%s</option>''' %(choice_item[0],selected,choice_item[1])
            selected=''
    if type(field_obj).__name__ in ["DateTimeField","DateFeild"]:
        date_els=[]
...
def get_m2m_obj_list(admin_class ,field ,form_obj):
    ...
    """
    #表的对象的字段
    field_obj=getattr(admin_class.model,form_obj)
